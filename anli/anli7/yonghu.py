...
class SensorCollect:
    ...
    def __init__(self, connector, env_name: str , robot_name: str, namespace: str, message_type):      
        ...
        """        
        self.subscriber = connector.Subscribe(env_name, robot_name, '/' + env_name + '/' + robot_name + '/' + namespace, message_type, self.callback)
...
class CompressedImage(SensorCollect):
    def __init__(self, connector, env_name: str, robot_name: str):
        super().__init__(connector, env_name, robot_name, 'color/image_raw/compressed', CompressedImage_pb2.CompressedImage)
...
def main(args):
    ...
    # 传感例子
    img_example = Image(connector, 'NT2', 'JD1204_2')
